...
####################################################################################
# get prediction
Outputs_files_2020 <- list.files(paste0(path=grep("ET_Berlin2020_", # the same name and dir as the 2.2 simulations
...
Outputs_files_2020 <- gtools::mixedsort(sort(Outputs_files_2020))

# read all in a list
Outputs_list_2020 <- lapply(1:length(Outputs_files_2020), 
                            function(i)  readr::read_csv(Outputs_files_2020[i],
                                                         col_names = T,
                                                         readr::cols(.default = readr::col_double()))[-1,])

# stack all the 55 subsets # A tibble: 9,663,928 × 17
Berlin2020_pred <- dplyr::bind_rows(Outputs_list_2020)

Berlin2020_pred$timestamp <- BerkeleyJulianDateToPOSIXct(Inputs_Berlin_20$t, "UTC")
...
SCOPE_parameters_Berlin2020 <- get_parameters(
  ...
  Simulation_Name = "ET_Berlin2020_",
  LAI = TRUE,
  SMC = TRUE,
  hc = TRUE,
  hc_c = TRUE,

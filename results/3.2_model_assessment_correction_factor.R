...
########################################################################
## EC tower
EC_ROTH <- read_csv("C:/Users/Alby Rocha/Documents/EC/FPmaps/EC_ROTHh.csv",
                    col_names = T,
                    cols(
                      .default = col_double(),
...
summary(EC_ROTH)
# filter 2019
EC_ROTH19 <- filter(EC_ROTH, date(timestamp)>="2018-11-30" &
                             date(timestamp)<="2020-02-01")
#calculate day and night time
night <- getSunlightTimes(date = date(EC_ROTH19$timestamp[1:10225]), 
                          keep = c("nightEnd", "night"), 
                          lat = 52, lon = 13, tz = "UTC")

## SCOPE inputs based on weather DWD
Input_ROTH <- read_csv("D:/SCOPE-2.0N/SCOPE-2.0/input/dataset for_verification/Input_ROTH.csv",
                     col_names = TRUE,
                     cols(
                       .default = col_double(),
                       timestamp = col_datetime(format = "")))
...
## ETo calculation ##
########################################################################
# response variable to assess the model accuracy
ROTH_Results <- NULL
ROTH_Results$timestamp <- EC_ROTH19$timestamp[1:10225]
ROTH_Results$month <- month(EC_ROTH19$timestamp[1:10225], label=T)
ROTH_Results$day_night <- if_else(EC_ROTH19$timestamp[1:10225]>night$night | EC_ROTH19$timestamp[1:10225]<night$nightEnd, "night", "day")
...
ROTH_Results$ET.obsC[ROTH_Results$ET.obsC<=-0.0000000001] <- 0 # set negative values to zero
...
ROTH_Results$ETo  <- hourlyET(data.frame(wind=na.approx(Input_ROTH$u[1:10225]),
                                ...
                                height=39.75,  elev=50,
                                   lat=52.4537, long=13.3017,  tz = "UTC"), 
                              hours=hour(ymd_hms(Input_ROTH$timestamp[1:10225], tz="UTC")) , 
                              DOY=yday(ymd_hms(Input_ROTH$timestamp[1:10225], tz="UTC")), 
                              ET="ETo")

ROTH_Results$ETo[ROTH_Results$ETo<=-0.0000000001] <- 0 # set negative values to zero
# ETo from EC data
...
ROTH_Results$ETo.EC[ROTH_Results$ETo.EC<=-0.0000000001] <- 0# set negative values to zero

##################################################################
##### correction fraction ########################################
##################################################################
## Layers from the urban atlas extracted by FP 
FP_ROTH <- read_csv("C:/Users/Alby Rocha/Documents/EC/FPmaps/fp_ROTHnewh.csv",
                    col_names = TRUE,
...
summary(FP_ROTH)
##################################################################
#veg.cover  :   "Veg_cover"/100, "Veg_cover_str"/100,
#                "Green_volume", "veg_cover_vh_old"
#impervious : "Impervious"/100 , "Impervious_With_str"/100,
...
#######################################################################
# different correction factor approaches
...
ROTH_Results$ETo_adjAN <- ifelse(ROTH_Results$day_night=="night", ROTH_Results$ETo,
                            ROTH_Results$ETo*na.approx(FP_ROTH$veg_cover_vh_old[1:10225]))
ROTH_Results$ETo_adjBN <- ifelse(ROTH_Results$day_night=="night",ROTH_Results$ETo,
                           ROTH_Results$ETo*na.approx(FP_ROTH$Veg_cover[1:10225]/100))
ROTH_Results$ETo_adjCN <- ifelse(ROTH_Results$day_night=="night", ROTH_Results$ETo,
                           ROTH_Results$ETo*na.approx(1-(FP_ROTH$impervious_old[1:10225])))
ROTH_Results$ETo_adjDN <- ifelse(ROTH_Results$day_night=="night", ROTH_Results$ETo,
...
Metric_ROTH <- list(
  data.frame(
    ...
    "rbias"= sum(ROTH_Results$ETo-ROTH_Results$ET.obsC,na.rm=T)/sum(ROTH_Results$ET.obsC,na.rm=T),row.names = "ETo"),
  data.frame(
    ...
    "rbias"= sum(ROTH_Results$ETo_adjA-ROTH_Results$ET.obsC,na.rm=T)/sum(ROTH_Results$ET.obsC,na.rm=T),row.names = "ETo_adjA"),
  data.frame(
    ...
    "rbias"= sum(ROTH_Results$ETo_adjB-ROTH_Results$ET.obsC,na.rm=T)/sum(ROTH_Results$ET.obsC,na.rm=T),row.names = "ETo_adjB"),
  data.frame(
    ...
    "rbias"= sum(ROTH_Results$ETo.EC_adjB-ROTH_Results$ET.obsC,na.rm=T)/sum(ROTH_Results$ET.obsC,na.rm=T),row.names = "ETo.EC_adjB"),
  data.frame(
    ...
    "rbias"= sum(ROTH_Results$ETo_adjC-ROTH_Results$ET.obsC,na.rm=T)/sum(ROTH_Results$ET.obsC,na.rm=T),row.names = "ETo_adjC"),
  data.frame(
    ...
    "rbias"= sum(ROTH_Results$ETo_adjD-ROTH_Results$ET.obsC,na.rm=T)/sum(ROTH_Results$ET.obsC,na.rm=T), row.names = "ETo_adjD"),
  data.frame(
    ...
    "rbias"= sum(ROTH_Results$ETo_adjAN-ROTH_Results$ET.obsC,na.rm=T)/sum(ROTH_Results$ET.obsC,na.rm=T),row.names = "ETo_adjAN"),
  data.frame(
    ...
    "rbias"= sum(ROTH_Results$ETo_adjBN-ROTH_Results$ET.obsC,na.rm=T)/sum(ROTH_Results$ET.obsC,na.rm=T),row.names = "ETo_adjBN"),
  data.frame(
    ...
    "rbias"= sum(ROTH_Results$ETo_adjCN-ROTH_Results$ET.obsC,na.rm=T)/sum(ROTH_Results$ET.obsC,na.rm=T),row.names = "ETo_adjCN"),
  data.frame(
    ...
    "rbias"= sum(ROTH_Results$ETo_adjDN-ROTH_Results$ET.obsC,na.rm=T)/sum(ROTH_Results$ET.obsC,na.rm=T),row.names = "ETo_adjDN"))

ETo_ROTH <- matrix(unlist(Metric_ROTH), nrow=3)

colnames(ETo_ROTH) <- c("ETo_orig","ETo_AdjA","ETo_AdjB", "ETo.EC_AdjB","ETo_AdjC","ETo_AdjD","AdjAN","AdjBN","AdjCN","AdjDN")
...
for (i in 1:length(SCOPE_ROTH)) {
SCOPE_ET_ROTH[[i]]$SCOPE_ET_adjA <- SCOPE_ET_ROTH[[i]]$SCOPE_ET*na.approx(FP_ROTH$veg_cover_vh_old[1:10225])
SCOPE_ET_ROTH[[i]]$SCOPE_ET_adjB <- SCOPE_ET_ROTH[[i]]$SCOPE_ET*na.approx(FP_ROTH$Veg_cover[1:10225]/100)
SCOPE_ET_ROTH[[i]]$SCOPE_ET_adjC <- SCOPE_ET_ROTH[[i]]$SCOPE_ET*na.approx(1-(FP_ROTH$impervious_old[1:10225]))
SCOPE_ET_ROTH[[i]]$SCOPE_ET_adjD <- SCOPE_ET_ROTH[[i]]$SCOPE_ET*na.approx(1-(FP_ROTH$Impervious_With_str[1:10225]/100))
...
##########################################################################
#### calculate the metric for SCOPE ######################################
##########################################################################
...
for (i in 1:length(SCOPE_ET_ROTH)) {
  ...
  matrix(c(defaultSummary(na.omit(data.frame(
           ...
           sum(filter(SCOPE_ET_ROTH[[i]], year(timestamp)==2019)[[2]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
              sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T), #Rbias
           defaultSummary(na.omit(data.frame(
             obs=filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,
           ...
           sum(filter(SCOPE_ET_ROTH[[i]], year(timestamp)==2019)[[5]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
           ...
           sum(filter(SCOPE_ET_ROTH[[i]], year(timestamp)==2019)[[6]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
             sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),        
           defaultSummary(na.omit(data.frame(
             obs=filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,
           ...
           sum(filter(SCOPE_ET_ROTH[[i]], year(timestamp)==2019)[[7]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
             sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
           defaultSummary(na.omit(data.frame(
             obs=filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,
           ...
           sum(filter(SCOPE_ET_ROTH[[i]], year(timestamp)==2019)[[8]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
             sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
           defaultSummary(na.omit(data.frame(
             obs=filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,
           ...
           sum(filter(SCOPE_ET_ROTH[[i]], year(timestamp)==2019)[[9]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
             sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
           defaultSummary(na.omit(data.frame(
             obs=filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,
           ...
           sum(filter(SCOPE_ET_ROTH[[i]], year(timestamp)==2019)[[10]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
             sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
           defaultSummary(na.omit(data.frame(
             obs=filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,
           ...
           sum(filter(SCOPE_ET_ROTH[[i]], year(timestamp)==2019)[[11]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
             sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
           defaultSummary(na.omit(data.frame(
             obs=filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,
           ...
           sum(filter(SCOPE_ET_ROTH[[i]], year(timestamp)==2019)[[12]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
             sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)), nrow=3))

colnames(SCOPE_Metric_ROTH[[i]][[1]]) <- c("SCOPE_orig","AdjA","AdjB","AdjC","AdjD","AdjAC","AdjAD","AdjBC","AdjBD")
...
colnames(day_night_ROTH) <- c("RMSE_night","RMSE_day","R2_night","R2_day","Rbias_night","Rbias_day")
                                         
t(round(day_night_ROTH,3))

# size of the FP - n pixel 
...
colnames(metric_FP_ROTH) <- c("RMSE<500p","RMSE_b","RMSE>2000p","R2<500p","R2_b","R2>2000p","Rbias<500p","Rbias_b","Rbias>2000p")

t(round(metric_FP_ROTH,3))

###################################################################################
# Correcting night and day separatly (only day)
###################################################################################
SCOPE_DN_ROTH <- NULL
...
for (i in 1:length(SCOPE_ROTH)) {
  SCOPE_DN_ROTH[[i]]$SCOPE_ET_adjA <- ifelse(SCOPE_DN_ROTH[[i]]$day_night=="night",SCOPE_DN_ROTH[[i]]$SCOPE_ET,
                                             SCOPE_DN_ROTH[[i]]$SCOPE_ET*na.approx(FP_ROTH$veg_cover_vh_old[1:10225]))
  SCOPE_DN_ROTH[[i]]$SCOPE_ET_adjB <- ifelse(SCOPE_DN_ROTH[[i]]$day_night=="night",SCOPE_DN_ROTH[[i]]$SCOPE_ET,
                                             SCOPE_DN_ROTH[[i]]$SCOPE_ET*na.approx(FP_ROTH$Veg_cover[1:10225]/100))
  SCOPE_DN_ROTH[[i]]$SCOPE_ET_adjC <- ifelse(SCOPE_DN_ROTH[[i]]$day_night=="night", SCOPE_DN_ROTH[[i]]$SCOPE_ET,
                                             SCOPE_DN_ROTH[[i]]$SCOPE_ET*na.approx(1-(FP_ROTH$impervious_old[1:10225])))
  SCOPE_DN_ROTH[[i]]$SCOPE_ET_adjD <- ifelse(SCOPE_DN_ROTH[[i]]$day_night=="night",SCOPE_DN_ROTH[[i]]$SCOPE_ET,
                                             SCOPE_DN_ROTH[[i]]$SCOPE_ET*na.approx(1-(FP_ROTH$Impervious_With_str[1:10225]/100)))
  SCOPE_DN_ROTH[[i]]$SCOPE_ET_adjAC <- ifelse(SCOPE_DN_ROTH[[i]]$day_night=="night",SCOPE_DN_ROTH[[i]]$SCOPE_ET,
                                              SCOPE_DN_ROTH[[i]]$SCOPE_ETsoil*na.approx(1-(FP_ROTH$impervious_old[1:10225]))+
                                                SCOPE_DN_ROTH[[i]]$SCOPE_ETcanopy*na.approx(FP_ROTH$veg_cover_vh_old[1:10225]))
  SCOPE_DN_ROTH[[i]]$SCOPE_ET_adjAD <- ifelse(SCOPE_DN_ROTH[[i]]$day_night=="night",SCOPE_DN_ROTH[[i]]$SCOPE_ET,
                                              SCOPE_DN_ROTH[[i]]$SCOPE_ETsoil*na.approx(1-(FP_ROTH$Impervious_With_str[1:10225]/100))+
                                                SCOPE_DN_ROTH[[i]]$SCOPE_ETcanopy*na.approx(FP_ROTH$veg_cover_vh_old[1:10225]))
  SCOPE_DN_ROTH[[i]]$SCOPE_ET_adjBC <- ifelse(SCOPE_DN_ROTH[[i]]$day_night=="night",SCOPE_DN_ROTH[[i]]$SCOPE_ET,
                                              SCOPE_DN_ROTH[[i]]$SCOPE_ETsoil*na.approx(1-(FP_ROTH$impervious_old[1:10225]))+
                                                SCOPE_DN_ROTH[[i]]$SCOPE_ETcanopy*na.approx(FP_ROTH$Veg_cover[1:10225]/100))
  SCOPE_DN_ROTH[[i]]$SCOPE_ET_adjBD <- ifelse(SCOPE_DN_ROTH[[i]]$day_night=="night",SCOPE_DN_ROTH[[i]]$SCOPE_ET,
                                              SCOPE_DN_ROTH[[i]]$SCOPE_ETsoil*na.approx(1-(FP_ROTH$Impervious_With_str[1:10225]/100))+
                                                SCOPE_DN_ROTH[[i]]$SCOPE_ETcanopy*na.approx(FP_ROTH$Veg_cover[1:10225]/100))
}
...
for (i in 1:length(SCOPE_DN_ROTH)) {
  SCOPE_MetricDN_ROTH[[i]] <- list(
    matrix(c(defaultSummary(na.omit(data.frame(
      ...
      sum(filter(SCOPE_DN_ROTH[[i]], year(timestamp)==2019)[[2]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
        sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
      ...
      sum(filter(SCOPE_DN_ROTH[[i]], year(timestamp)==2019)[[5]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
        sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
      ...
      sum(filter(SCOPE_DN_ROTH[[i]], year(timestamp)==2019)[[6]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
        sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
      ...
      sum(filter(SCOPE_DN_ROTH[[i]], year(timestamp)==2019)[[7]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
        sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
      ...
      sum(filter(SCOPE_DN_ROTH[[i]], year(timestamp)==2019)[[8]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
        sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
      ...
      sum(filter(SCOPE_DN_ROTH[[i]], year(timestamp)==2019)[[9]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
        sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
      ...
      sum(filter(SCOPE_DN_ROTH[[i]], year(timestamp)==2019)[[10]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
        sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
      ...
      sum(filter(SCOPE_DN_ROTH[[i]], year(timestamp)==2019)[[11]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
        sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T),
      ...
      sum(filter(SCOPE_DN_ROTH[[i]], year(timestamp)==2019)[[12]]-filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)/
        sum(filter(ROTH_Results, year(timestamp)==2019)$ET.obsC,na.rm=T)), nrow=3))
  
  colnames(SCOPE_MetricDN_ROTH[[i]][[1]]) <- c("SCOPE_orig","AdjA","AdjB","AdjC","AdjD","AdjAC","AdjAD","AdjBC","AdjBD")
...
###############################################################################
###############################################################################
## model selection and assessiment by the scenarios############################
###############################################################################
###############################################################################
#ETo
t(round(ETo_ROTH,3))
